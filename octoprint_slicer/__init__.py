...
class SlicerPlugin(octoprint.plugin.SettingsPlugin,
	...
	def get_assets(self):
		...
		# core UI here.
		return dict(
			js=["js/slicer.js", "js/three.min.js", "js/STLLoader.js", "js/OrbitControls.js", "js/TransformControls.js", "js/Detector.js", "js/OrbitControls.js", "js/TransformControls.js", "js/STLBinaryExporter.js"],
			css=["css/slicer.css"],
	...
	def fileCommand(filename, target):
		...
		if command == "slice":
			try:
				if "slicer" in data:
					slicer = data["slicer"]
					del data["slicer"]
					slicer_instance = self._slicingManager.get_slicer(slicer)

				elif "cura" in self._slicingManager.registered_slicers:
					slicer = "cura"
					slicer_instance = self._slicingManager.get_slicer("cura")
